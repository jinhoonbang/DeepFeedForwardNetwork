...
class SoftMax(object):
    ...
    def __init__(self, input, n_in, n_out):
        ...
        self.y_pred = T.concatenate([b1, b2 ,b3, b4, b5, b6,b7,b8,b9,b10,b11,b12,b13,b14,b15,b16,b17,b18,b19,b20,b21,b22,b23,b24,b25,b26,b27,b28,b29,b30,b31,b32,b33,b34,b35,b36,b37,b38,b39,b40,b41,b42,b43], axis=0)

        # b = T.exp(T.dot(input. self.W) + self.b)
        # b1 = b[:, :3] / b[:, :3].sum(axis=1)[:, None]
        # b2 = b[:, 3:6] / b[:, 3:6].sum(axis=1)[:, None]
        # b3 = b[:, 6:9] / b[:, 6:9].sum(axis=1)[:, None]
        # b4 = b[:, 9:12] / b[:, 9:12].sum(axis=1)[:, None]
        # b5 = b[:, 12:15] / b[:, 12:15].sum(axis=1)[:, None]
        # b6 = b[:, 15:18] / b[:, 15:18].sum(axis=1)[:, None]
        # b7 = b[:, 18:21] / b[:, 18:21].sum(axis=1)[:, None]
        # b8 = b[:, 21:24] / b[:, 21:24].sum(axis=1)[:, None]
        # b9 = b[:, 24:27] / b[:, 24:27].sum(axis=1)[:, None]
        # b10 = b[:, 27:30] / b[:, 27:30].sum(axis=1)[:, None]
        # b11 = b[:, 30:33] / b[:, 30:33].sum(axis=1)[:, None]
        # b12 = b[:, 33:36] / b[:, 33:36].sum(axis=1)[:, None]
        # b13 = b[:, 36:39] / b[:, 36:39].sum(axis=1)[:, None]
        # b14 = b[:, 39:42] / b[:, 39:42].sum(axis=1)[:, None]
        # b15 = b[:, 42:45] / b[:, 42:45].sum(axis=1)[:, None]
        # b16 = b[:, 45:48] / b[:, 45:48].sum(axis=1)[:, None]
        # b17 = b[:, 48:51] / b[:, 48:51].sum(axis=1)[:, None]
        # b18 = b[:, 51:54] / b[:, 51:54].sum(axis=1)[:, None]
        # b19 = b[:, 54:57] / b[:, 54:57].sum(axis=1)[:, None]
        # b20 = b[:, 57:60] / b[:, 57:60].sum(axis=1)[:, None]
        # b21 = b[:, 60:63] / b[:, 60:63].sum(axis=1)[:, None]
        # b22 = b[:, 63:66] / b[:, 63:66].sum(axis=1)[:, None]
        # b23 = b[:, 66:69] / b[:, 66:69].sum(axis=1)[:, None]
        # b24 = b[:, 69:72] / b[:, 69:72].sum(axis=1)[:, None]
        # b25 = b[:, 72:75] / b[:, 72:75].sum(axis=1)[:, None]
        # b26 = b[:, 75:78] / b[:, 75:78].sum(axis=1)[:, None]
        # b27 = b[:, 78:81] / b[:, 78:81].sum(axis=1)[:, None]
        # b28 = b[:, 81:84] / b[:, 81:84].sum(axis=1)[:, None]
        # b29 = b[:, 84:87] / b[:, 84:87].sum(axis=1)[:, None]
        # b30 = b[:, 87:90] / b[:, 87:90].sum(axis=1)[:, None]
        # b31 = b[:, 90:93] / b[:, 90:93].sum(axis=1)[:, None]
        # b32 = b[:, 93:96] / b[:, 93:96].sum(axis=1)[:, None]
        # b33 = b[:, 96:99] / b[:, 96:99].sum(axis=1)[:, None]
        # b34 = b[:, 99:102] / b[:, 99:102].sum(axis=1)[:, None]
        # b35 = b[:, 102:105] / b[:, 102:105].sum(axis=1)[:, None]
        # b36 = b[:, 105:108] / b[:, 105:108].sum(axis=1)[:, None]
        # b37 = b[:, 108:111] / b[:, 108:111].sum(axis=1)[:, None]
        # b38 = b[:, 111:114] / b[:, 111:114].sum(axis=1)[:, None]
        # b39 = b[:, 114:117] / b[:, 114:117].sum(axis=1)[:, None]
        # b40 = b[:, 117:120] / b[:, 117:120].sum(axis=1)[:, None]
        # b41 = b[:, 120:123] / b[:, 120:123].sum(axis=1)[:, None]
        # b42 = b[:, 123:126] / b[:, 123:126].sum(axis=1)[:, None]
        # b43 = b[:, 126:129] / b[:, 126:129].sum(axis=1)[:, None]
        #
        # self.p_y_given_x =
        #
        # stockCount = 43
        #
        # cycle = int(1612500 / stockCount)
